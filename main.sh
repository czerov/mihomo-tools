#!/bin/bash
# main.sh - Mihomo 命令行管理工具 v1.0.3 + CPU V3 自检
# 安装路径: /usr/bin/mihomo
    ...
    case $ARCH in
        x86_64)
            ...
            if grep -q "avx2" /proc/cpuinfo; then
                echo -e "${GREEN}✅ 检测到 CPU 支持 x86_64-v3 指令集${NC}"
                DEFAULT_V="1"
            else
                echo -e "${YELLOW}⚠️ 检测到 CPU 不支持 v3 指令集${NC}"
                DEFAULT_V="2"
            fi
            echo "1) 高性能版 (amd64)"
            echo "2) 兼容版 (amd64-compatible)"
            read -p "选择版本 [默认: $DEFAULT_V]: " CPU_CHOICE
