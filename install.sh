#!/bin/bash
# install.sh - v1.0.6 最终兼容全量版 + CPU V3 自检
# 功能：自动补齐 iptables、安全加载 .env、去 TG 化、自动识别 CPU 指令集
...
case $ARCH in
    x86_64)
        echo -e "\n🔍 正在检测 CPU 指令集支持情况..."
        # 常见 x86_64-v3 依赖 avx2，以此作为检测标准
        if grep -q "avx2" /proc/cpuinfo; then
            echo "✅ 检测到 CPU 支持 x86_64-v3 指令集 (推荐: 高性能版)"
            DEFAULT_V="1"
        else
            echo "⚠️ 检测到 CPU 不支持 v3 指令集 (或 PVE 未开启 host 模式，推荐: 兼容版)"
            DEFAULT_V="2"
        fi
        echo "请选择要安装的内核版本:"
        echo "1) 高性能版 (amd64) - 需要 CPU 硬件支持"
        echo "2) 兼容版 (amd64-compatible) - 适合老旧 CPU 或默认 KVM 虚拟机"
        read -p "请输入 [默认: $DEFAULT_V]: " CPU_CHOICE
        ...
        URL="https://github.com/MetaCubeX/mihomo/releases/download/${LATEST_VER}/mihomo-${PLATFORM}-${LATEST_VER}.gz"
        ;;
